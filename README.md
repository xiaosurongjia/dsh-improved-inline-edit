...
- 撤回功能：消息气泡旁「撤回」按钮（带外框胶囊 UI），未被读取时可撤回，已读取则变灰。
- 草稿持久化：输入内容实时存入 localStorage（按 sessionId），不丢稿。
- 任务结束草稿保留：有内容则输入条保留，发送或清空后消失。
- 注入引导前缀：发送按钮左侧小方块开关（开=勾选），消息加「继续原任务」引导。

v0.1.6

- 按钮推至输入框最右侧，颜色统一走 DSH 主题 token。

v0.1.5

- textarea 自动换行增高（上限 140px），超限显示简约滚动条。

v0.1.4

- 修复 `setTimeout` 崩溃（改 timer 服务）；黄色实心圆钮动画。

v0.1.3

- 修复与 composer 左对齐；箭头向上；仅运行中显示。
...
- 接入 deep 短语池（73 条）轮换 + 中英切换。

v0.1.0

- 首个可发布版本：运行中弹出「修改要求」输入条，`agent.steer()` 注入。
